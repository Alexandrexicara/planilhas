...
class TelaLoginPrincipal:
    def __init__(self):
        self.janela = tk.Tk()
        self.janela.title("planilhas.com - Sistema Completo com PagBank")
    ...
    def criar_interface(self):
        ...
        btn_cadastro = ttk.Button(frame_opcoes, text="🆕 Novo Cadastro + Pagamento PIX", 
        ...
        btn_testar = ttk.Button(frame_opcoes, text="🧪 Testar PagBank", 
        ...
        info = ttk.Label(frame_principal, 
                        text="Novos usuários: Cadastre-se e pague via PIX para acessar\nUsuários existentes: Use login tradicional",
                        font=("Arial", 9), foreground='gray', justify='center')
        info.pack(pady=(20, 0))
        
        # Status PagBank
        self.status_label = ttk.Label(frame_principal, 
                                   text="Verificando PagBank...", 
                                   font=("Arial", 8), foreground='gray')
        self.status_label.pack(pady=(10, 0))
        
        # Verificar status em background
        self.verificar_status_pagbank()
    
    def verificar_status_pagbank(self):
        """Verifica status da conexão com PagBank"""
        try:
            token = self.sistema_pg.obter_token_pagbank()
            if token:
                self.status_label.config(text="🟢 PagBank Conectado", foreground='green')
            else:
                self.status_label.config(text="🔴 PagBank Offline", foreground='red')
        except Exception as e:
            self.status_label.config(text=f"🔴 Erro: {str(e)[:20]}...", foreground='red')
    ...
    def testar_pagbank(self):
        ...
        try:
            from testar_pagbank import testar_pagbank
            testar_pagbank()
            messagebox.showinfo("Teste", "Testes PagBank executados! Verifique o console.")
        except Exception as e:
    ...
    def fazer_login_direto(self):
        ...
        try:
            # Tentar login no sistema de pagamento PagBank primeiro
