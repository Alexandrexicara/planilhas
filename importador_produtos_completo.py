...
class ImportadorProdutosCompleto:
    def __init__(self):
    ...
    def limpar_dados(self, texto):
        """Limpa e formata texto preservando caracteres especiais"""
        if pd.isna(texto) or texto is None:
            return ""
        
        texto = str(texto).strip()
        
        # Preservar caracteres especiais importantes (acentos, símbolos)
        # Remover apenas caracteres de controle problemáticos
        texto = re.sub(r'[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]', '', texto)
        
        # Substituir quebras de linha por espaços para evitar problemas
        texto = texto.replace('\n', ' ').replace('\r', ' ')
        
        # Limitar tamanho razoável para descrições longas
        if len(texto) > 1000:
            texto = texto[:1000] + "..."
    ...
    def converter_numero(self, valor):
        ...
        try:
            ...
            # Se começar com =, é fórmula do Excel - tentar calcular
    ...
    def processar_imagem(self, imagem_path):
        ...
        if pd.isna(imagem_path) or imagem_path is None or imagem_path == '':
            return None, None
        
        try:
            # Se for caminho de arquivo
            if os.path.exists(str(imagem_path)):
                with open(str(imagem_path), 'rb') as img_file:
                    imagem_base64 = base64.b64encode(img_file.read()).decode('utf-8')
                    return str(imagem_path), imagem_base64
            
            # Se já for base64
            if str(imagem_path).startswith('data:image'):
                return None, str(imagem_path)
                
            return str(imagem_path), None
            
        except Exception as e:
            print(f"   Aviso: Erro ao processar imagem {imagem_path}: {str(e)}")
            return str(imagem_path), None
    
    def importar_dados(self, arquivo_excel):
        """Importa dados do arquivo Excel preservando imagens"""
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    # Procurar colunas de imagem
                    for col in ['imagem', 'image', 'foto', 'photo', 'picture']:
                        if col in row and not pd.isna(row[col]):
                            imagem_path, imagem_base64 = self.processar_imagem(row[col])
                            break
                    ...
                    INSERT INTO produtos (
                        ...
                        prc_kg, li, obs, status, imagem_path, imagem_base64, data_importacao
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
                    ...
                    if produtos_importados % 5 == 0:
                        ...
                        if imagem_path:
                            print(f"   Com imagem: {imagem_path}")
            ...
            self.cursor.execute("SELECT COUNT(*) FROM produtos WHERE imagem_path IS NOT NULL")
            com_imagem = self.cursor.fetchone()[0]
...
def testar_importacao_completa():
    """Testa a importacao com dados simulados incluindo imagens"""
    print("Testando importacao completa de produtos...")
    
    importador = ImportadorProdutosCompleto()
    ...
    dados_teste = [
        {
            ...
            'descricao': 'PAPEL DE PAREDE ADESIVO ESTAMPA ESTRELADO 5 METROS - Medida: 5m x 45cm - Cor: Branco - Material: Policloreto de Vinil',
            'marca': 'WESTERN HOME',
            'inner_qty': '',
            'master_qty': '30',
            'total_ctns': 120,
            ...
            'obs': 'NCM OK',
            'status': 'REPOSIÇÃO',
            'imagem': 'imagens/papel_parede_estrelado.jpg'
        },
        {
            'doc': '5266',
            'rev': '1',
            ...
            'ccy': 'USD',
            'valor': 0.56,
            'total_amount': '1108.80',
            'descricao': 'PAPEL DE PAREDE ADESIVO ESTAMPA DE MADEIRA 5 METROS - Medida: 5m x 45cm - Cor: Marrom - Material: Policloreto de Vinil',
            'marca': 'WESTERN HOME',
            'inner_qty': '',
            'master_qty': '30',
            'total_ctns': 66,
            ...
            'gross_weight': 633.6,
            'net_weight_pc': '',
            'gross_weight_pc': 0.377,
            'net_weight_ctn': 8.643,
            'gross_weight_ctn': 9.6,
            'factory': 'YOUNGCOM HONGKONG LIMITED',
            'address': 'YIWU,CHINA',
            'telephone': '',
            'ean13': '7897186068220',
            'dun14_inner': '',
            'dun14_master': '57897186068225',
            'length': 25,
            'width': 20.5,
            'height': 47,
            'cbm': 1.5898,
            ...
            'obs': 'NCM OK',
            'status': 'REPOSIÇÃO',
            'imagem': 'imagens/papel_parede_madeira.jpg'
        }
    ]
    ...
    try:
        ...
        for dados in dados_teste:
            # Processar imagem
            imagem_path, imagem_base64 = importador.processar_imagem(dados.get('imagem'))
            ...
            INSERT INTO produtos (
                ...
                descricao, marca, inner_qty, master_qty, total_ctns, peso,
                gross_weight, net_weight_pc, gross_weight_pc, net_weight_ctn,
                gross_weight_ctn, factory, address, telefone, ean13,
                dun14_inner, dun14_master, length, width, height, cbm,
                prc_kg, li, obs, status, imagem_path, imagem_base64, data_importacao
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ...
                dados['gross_weight_pc'], dados['net_weight_ctn'],
                dados['gross_weight_ctn'], dados['factory'], dados['address'],
                dados.get('telephone', ''), dados['ean13'], dados['dun14_inner'],
                dados['dun14_master'], dados['length'], dados['width'],
                dados['height'], dados['cbm'], dados['prc_kg'],
                dados['li'], dados['obs'], dados['status'],
                imagem_path, imagem_base64,
                datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            ))
        ...
        importador.cursor.execute("SELECT codigo, descricao, imagem_path FROM produtos")
        produtos = importador.cursor.fetchall()
        
        print("\nProdutos inseridos:")
        for produto in produtos:
            print(f"  {produto[0]} - {produto[1][:50]}...")
            if produto[2]:
                print(f"    Imagem: {produto[2]}")
