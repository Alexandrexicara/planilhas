...
def inicializar_banco():
    ...
    cursor.execute("PRAGMA cache_size=5000")       # Cache menor
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_cliente ON produtos(cliente)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_codigo ON produtos(codigo)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_descricao ON produtos(descricao)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_ncm ON produtos(ncm)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_busca_geral ON produtos(cliente, codigo, descricao, ncm)")
...
def importar_planilha(caminho_arquivo, cliente=None, progress_callback=None):
    """Importa uma única planilha (versão otimizada para alta capacidade)"""
    if not cliente:
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            if len(dados_batch) >= batch_size:
                cursor.executemany("""
                    ...
                    (cliente, arquivo_origem, codigo, descricao, peso, valor, ncm, data_importacao)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, dados_batch)
                conn.commit()
        ...
        cursor.execute("""
            INSERT INTO importacoes (arquivo, data_hora, produtos_importados)
            VALUES (?, ?, ?)
        """, (os.path.basename(caminho_arquivo), data_atual, total_importados))
        conn.commit()
...
def detectar_colunas_excel(cabecalhos):
    ...
    mapeamento = {
        'codigo': ['codigo', 'código', 'cod', 'code', 'item', 'sku', 'referencia', 'referência'],
        'descricao': ['descricao', 'descrição', 'produto', 'item_desc', 'name', 'descrição do produto'],
        'peso': ['peso', 'weight', 'kg', 'quilos', 'peso_bruto', 'peso_liquido'],
        'valor': ['valor', 'preco', 'preço', 'price', 'unitario', 'unitário', 'custo'],
        'ncm': ['ncm', 'nomenclatura', 'codigo_ncm', 'ncm_sh', 'codigo_ncm_sh']
    }
...
def buscar_produtos(termo, cliente_filtro=None, limit=1000):
...
class SistemaPlanilhas:
    ...
    def criar_interface(self):
        ...
        frame_import_botoes = tk.Frame(frame_import, bg='#f0f0f0')
        frame_import_botoes.pack(pady=15)
        
        # Container para alinhamento perfeito dos botões
        frame_botoes_container = tk.Frame(frame_import_botoes, bg='#f0f0f0')
        frame_botoes_container.pack()
        
        # Primeira linha de botões
        frame_botoes_linha1 = tk.Frame(frame_botoes_container, bg='#f0f0f0')
        frame_botoes_linha1.pack(pady=(0, 8))
        
        # Container para os botões da primeira linha (igual estrutura)
        frame_linha1_container = tk.Frame(frame_botoes_linha1, bg='#f0f0f0')
        frame_linha1_container.pack()
        
        tk.Button(frame_linha1_container, text="📁 Importar Pasta 'planilhas/'", command=self.importar_pasta,
                 bg='#27ae60', fg='white', font=('Arial', 10, 'bold'), width=22, height=2,
                 relief='raised', bd=2, cursor='hand2').pack(side='left', padx=5)
        
        tk.Button(frame_linha1_container, text="📄 Selecionar Arquivos", command=self.selecionar_arquivos,
                 bg='#f39c12', fg='white', font=('Arial', 10, 'bold'), width=22, height=2,
                 relief='raised', bd=2, cursor='hand2').pack(side='left', padx=5)
        
        tk.Button(frame_linha1_container, text="🔄 Limpar Banco", command=self.limpar_banco,
                 bg='#e67e22', fg='white', font=('Arial', 10, 'bold'), width=22, height=2,
                 relief='raised', bd=2, cursor='hand2').pack(side='left', padx=5)
        
        # Segunda linha de botões
        frame_botoes_linha2 = tk.Frame(frame_botoes_container, bg='#f0f0f0')
        frame_botoes_linha2.pack()
        
        # Container para o botão capacidade (igual estrutura)
        frame_capacidade_container = tk.Frame(frame_botoes_linha2, bg='#f0f0f0')
        frame_capacidade_container.pack()
        
        tk.Button(frame_capacidade_container, text="📊 Capacidade do Sistema", command=self.mostrar_capacidade,
                 bg='#9b59b6', fg='white', font=('Arial', 10, 'bold'), width=35, height=2,
                 relief='raised', bd=2, cursor='hand2').pack(pady=(8, 0))
        
        # Terceira linha - Manual do Usuário
        frame_botoes_linha3 = tk.Frame(frame_botoes_container, bg='#f0f0f0')
        frame_botoes_linha3.pack(pady=(8, 0))
        
        # Container para o botão manual (igual estrutura dos outros)
        frame_manual_container = tk.Frame(frame_botoes_linha3, bg='#f0f0f0')
        frame_manual_container.pack()
        
        tk.Button(frame_manual_container, text="📖 Manual do Usuário", command=self.abrir_manual,
                 bg='#FF8C00', fg='white', font=('Arial', 11, 'bold'), width=40, height=2,
                 relief='raised', bd=3, cursor='hand2').pack()
        
        # Quarta linha - Sistema PLUS
        frame_botoes_linha4 = tk.Frame(frame_botoes_container, bg='#f0f0f0')
        frame_botoes_linha4.pack(pady=(8, 0))
        
        # Container para o botão PLUS
        frame_plus_container = tk.Frame(frame_botoes_linha4, bg='#f0f0f0')
        frame_plus_container.pack()
        
        tk.Button(frame_plus_container, text="🚀 Sistema PLUS", command=self.abrir_sistema_plus,
                 bg='#e74c3c', fg='white', font=('Arial', 11, 'bold'), width=35, height=2,
                 relief='raised', bd=3, cursor='hand2').pack()
        ...
        self.colunas = ('Cliente', 'Arquivo', 'Código', 'Descrição', 'Peso', 'Valor', 'NCM')
        ...
        for col in self.colunas:
            ...
            if col == 'Descrição':
                self.tabela.column(col, width=300)
            elif col == 'Cliente':
    ...
    def executar_busca(self):
        ...
        if not termo and cliente == "Todos":
            self.status_bar.config(text="Digite um termo para buscar ou selecione um cliente")
            return
